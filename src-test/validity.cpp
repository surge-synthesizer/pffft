/*
  Copyright (c) 2013 Julien Pommier, 2025 Surge Synth Team.

  Small test for PFFFT, comparing its performance with the scalar FFTPACK, FFTW, Intel MKL, and Apple vDSP

  Adapted to run as a Catch2 test.
 */
...
extern "C"
{
    ...
    bool pffft_validate_N(int N, int cplx)
    {
        ...
        for (pass = 0; pass < 2; ++pass)
        {
            ...
            for (k = 0; k < Nfloat; ++k)
                ref_max = MAX_OF(ref_max, fabs(ref[k]));
    ...
    bool pffft_validate(int cplx)
    {
        static int Ntest[] = {16,   32,   64,     96,   128,  160,     192,   256,
                              288,  384,  5 * 96, 512,  576,  5 * 128, 800,   864,
                              1024, 2048, 2592,   4000, 4096, 12000,   36864, 0};
        int k;
        for (k = 0; Ntest[k]; ++k)
        {
            int N = Ntest[k];
    ...
    void show_output(const char *name, int N, int cplx, float flops, float t0, float t1,
                     int max_iter)
    {
        ...
        else
        {
            if (flops != -1)
            {
                printf("N=%5d, %s %16s : %6.0f MFlops [t=%6.0f ns, %d runs]\n", N,
                       (cplx ? "CPLX" : "REAL"), name, mflops, (t1 - t0) / 2 / max_iter * 1e9,
                       max_iter);
            }
        }
        fflush(stdout);
    ...
    void benchmark_ffts(int N, int cplx)
    {
        ...
        float *X = (float *)pffft_aligned_malloc(Nbytes),
              *Y = (float *)pffft_aligned_malloc(Nbytes),
              *Z = (float *)pffft_aligned_malloc(Nbytes);
        ...
        // FFTPack benchmark
        {
            ...
            flops = (max_iter_ * 2) * ((cplx ? 5 : 2.5) * N * log((double)N) /
                                       M_LN2); // see http://www.fftw.org/speed/method.html
        ...
        if (N == (1 << log2N))
        {
            ...
            flops = (max_iter * 2) * ((cplx ? 5 : 2.5) * N * log((double)N) /
                                      M_LN2); // see http://www.fftw.org/speed/method.html
...
#ifdef HAVE_MKL
        {
            ...
            if (DftiCreateDescriptor(&fft_handle, DFTI_SINGLE, (cplx ? DFTI_COMPLEX : DFTI_REAL), 1,
                                     N) == 0)
            {
...
#ifdef HAVE_FFTW
        {
            ...
            int flags = (N < 40000 ? FFTW_MEASURE
                                   : FFTW_ESTIMATE); // measure takes a lot of time on largest ffts
            // int flags = FFTW_ESTIMATE;
            if (cplx)
            {
                planf = fftwf_plan_dft_1d(N, (fftwf_complex *)in, (fftwf_complex *)out,
                                          FFTW_FORWARD, flags);
                planb = fftwf_plan_dft_1d(N, (fftwf_complex *)in, (fftwf_complex *)out,
                                          FFTW_BACKWARD, flags);
            }
            else
            {
            ...
            flops = (max_iter * 2) * ((cplx ? 5 : 2.5) * N * log((double)N) /
                                      M_LN2); // see http://www.fftw.org/speed/method.html
            show_output((flags == FFTW_MEASURE ? "FFTW (meas.)" : " FFTW (estim)"), N, cplx, flops,
                        t0, t1, max_iter);
        }
#endif
    ...
    void validate_pffft_simd(); // a small function inside pffft.c that will detect compiler bugs
                                // with respect to simd instruction
#endif
}
...
TEST_CASE("Validity", "Tests PFFFT validity")
{
    ...
    validate_pffft_simd();
#endif
    REQUIRE(pffft_validate(1));
    REQUIRE(pffft_validate(0));
}
