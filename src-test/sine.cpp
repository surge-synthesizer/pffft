...
TEST_CASE("Dynamic Sine", "Dynamic Sine Wave Wrapper Equivalence")
{
    ...
    // Exercise the array and ensure they all come to the same value.
    auto timeA = fft.createTimeArray();
    auto freqA = fft.createFreqArray();
    std::span<FFT::Time> sTimeA(timeA.get(), timeA.get() + fft.size);
    std::span<FFT::Frequency> sFreqA(freqA.get(), freqA.get() + fft.spectrum_size);
    std::copy(time.begin(), time.end(), sTimeA.begin());
    fft.forward(sTimeA, sFreqA);
    fft.scale(sFreqA);
    for (std::size_t i = 0; i < fft.spectrum_size; i++)
    {
        REQUIRE_THAT(std::abs(freqA[i]), Catch::Matchers::WithinRel(std::abs(freqC[i])));
