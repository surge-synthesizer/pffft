...
// Contains an API that allocates its outputs, as well as an API that takes
// already-allocated arrays. For the tl;dr of how to use it, scroll down to the
// "FFT" class.
#pragma once
...
// Class for performing a Fourier transform. This class is not thread safe; it
// uses a work array. Different threads should have their own thread-local copy
// of the class. To have a FFT size known at *run time*, set N to
// std::dynamic_extent and call reset() or the constructor accordingly.
// Otherwise, the FFT size is fixed at compile time as size N.
template <typename T, std::size_t N> class FFT
{
  ...
  public:
    ...
    // Alignment requirement for inputs and outputs. SSE and co need 16 byte
    ...
    // Size of the frequency array for the FFT. Since the spectrum is a
    // std::complex which is twice the size of a real, when T is real and not
    // complex the spectrum type std::complex<T> needs to be half the size.
    // Changes if you call reset().
    std::size_t spectrum_size =
        (N == std::dynamic_extent
             ? N
             : (std::is_same_v<float, typename std::remove_cv<T>::type> ? N / 2 : N));
    // For non-dynamic FFTs that can't be reset().
    static constexpr std::size_t kConstSpectrumSize =
        std::is_same_v<float, typename std::remove_cv<T>::type> ? N / 2 : N;

    // Helper types for specifying correctly-sized arrays. Use these if you want
    // to create the correct array sizes for the FFT without doing any math
    // yourself.
    ...
    // create vectors at runtime for the FFT that are the correct size and
    // alignment without doing any math yourself.
    ...
    // The use_stack parameter explicitly tells the class whether to allocate the
    // work array on the stack or the heap. For small transforms (N < 16384 or
    // so), stack can be faster. However, threads can have small stacks, so it
    // doesn't hurt to use the heap instead if you're concerned. No allocation is
    // performed except during construction time and in resize(), so even if it
    // uses the heap you don't need to worry about allocations during the
    // operation.
    ...
    // Functions to provide pre-allocated vectors in the exactly correct sizes for
    // the FFT. This will cause an allocation when these functions are called, so
    // don't use it if that's bad for you.
    ...
    // Output is in canonical form, AKA the familiar array of interleaved complex
    // numbers: [bin0_real, bin0_complex, bin1_real, bin1_complex, ...]
    //
    // The result is unscaled; call the scale() method if needed.
    //
    // The TimeVector/FreqVector API will perform allocations. If you're in a
    // tight loop or otherwise need to avoid heap allocations, use the array API
    // instead. The array API will throw if the input and output pointers are
    // improperly aligned.
    FreqVector forward(const TimeVector &time);
    // Alternate API to use with preallocated storage, such as that created by
    // this class or existing arrays. The time span must have N elements, and the
    // freq span must have spectrum_size elements. These must obey the alignment
    // requirements of SSE, or you'll get a runtime exception.
    void forward(std::span<const T> time, std::span<Complex> freq);
    // Raw pointer API. time must have N elements, and freq must have
    // spectrum_size elements. These must obey the alignment requires of SSE, or
    ...
    // Alternate API to use with preallocated storage, such as that created by
    // this class or existing arrays. The freq span must have spectrum_size
    // elements, and the time span must have N elements.
    void inverse(std::span<const Complex> freq, std::span<T> time);
    // Raw pointer API. freq must have spectrum_size elements, and time must have
    // N elements.
    ...
    //
    // All arguments must be SSE-aligned or there will be a runtime exception.
    ...
    requires(N != std::dynamic_extent)
{
    ...
    if (!use_stack)
    {
        // We use the aligned_allocator to create and destroy the work array,
        // instead of the regular aligned new[], because of a bug on MSVC (compiler
        // error C2956). This works around it.
